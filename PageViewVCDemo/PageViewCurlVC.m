 ...
 *                              最基本、最重要的四个方法
     1，无论翻页是通过点击手势、还是滑动手势：
        doubleSided = YES:
            中间部分翻页时调用顺序：2，2，1，3
            首页（继续上翻）、末页（继续下翻）翻页时调用顺序：2（注意不是2，2）
        doubleSided = NO:
            中间部分翻页时调用顺序：2，1，3
            首页（继续上翻）、末页（继续下翻）翻页时调用顺序：2
     2，PVC curl类型的重大Bug:当输入竖直方向手势时（不一定是垂直，只要是偏向于竖直方向即可），页面并不会进行动画翻页效果，但是会调用before方法（无论是向上的还是向下的竖直方向的手势），虽然页面显示正常，但是某些时候我们的逻辑值会因为这个Bug而改变（小说项目就是如此，目前还未找到解决办法）
            中间部分：调用顺序：单页：2；双叶：2，2。这种情况会造成逻辑值混乱，详情参考小说
            首页（继续上翻）、末页（继续下翻）：调用顺序：2（单、双都只有一次before）。这种情况一般没什么影响
 */
//开始翻页 ---1
